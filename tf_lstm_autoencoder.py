...
class LSTMAutoEncoder(tf.keras.Model):
    ...
    def call(self, inputs, training=None):
        if training is None:
            print("Training argument cannot be None in LSTMAutoEncoder")
            #return
...
def test_lstm_autoencoder(time_steps, latent_dim, num_layers, dropout, batch_size, epochs, csv_path):
    ...
    model.fit([X_sN, np.flip(X_sN, axis=1)], X_sN, epochs=epochs, batch_size=batch_size, validation_data=([X_vN2, np.flip(X_vN2, axis=1)], X_vN2), verbose=1, callbacks=[early_stopping])
