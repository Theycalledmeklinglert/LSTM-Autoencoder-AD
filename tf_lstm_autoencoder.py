...
def test_lstm_autoencoder(time_steps, layer_dims, dropout, batch_size, epochs, remove_timestamps, directories,
    ...
    for file_pair in all_file_pairs:
        ...
        flattened_X_vN_error_vecs = X_vN_error_vecs.reshape(X_vN_error_vecs.shape[0], X_vN_error_vecs.shape[1] * X_vN_error_vecs.shape[2])
        print(X_vN_error_vecs[0])
        print(flattened_X_vN_error_vecs[0])
        ...
        mu, sigma = estimate_normal_error_distribution(flattened_X_vN_error_vecs)
        X_vNA_error_vecs = calculate_rec_error_vecs(model, X_vNA, scaler)
        X_vNA_anomaly_scores = compute_anomaly_score(X_vNA_error_vecs, mu, sigma)  #todo: X_vN2 needs to be appended somewhere here and put into calc febeta as well
        ...
        #     if flattened_X_vNA_labels[index] == 1 or (
        #             flattened_X_vNA_labels[index] == 1 and flattened_X_vNA_labels[index + 1] == 0 and index <= len(
        #             flattened_X_vNA_labels) - 1):
        ...
        #         print("index of anom: " + str(index) + " | " + "anomaly score: " + str(X_vNA_anomaly_scores[index]))
        #         print("previous score: " + str(X_vNA_anomaly_scores[index - 1]))
        #         print("15 following scores: " + str(X_vNA_anomaly_scores[index:index + 15]))

        # todo: optimal value of beta seems to be application knowledge
        best_anomaly_threshold, best_fbeta = find_optimal_threshold(X_vNA_anomaly_scores, true_labels_list[2].flatten(), 0.9)
        ...
        X_vN_error_vecs = calculate_rec_error_vecs(model, data_with_time_diffs[1], scaler)
        X_vN_anomaly_scores = compute_anomaly_score(X_vN_error_vecs, mu, sigma)

        calculate_and_plot_detection_rate("X_vN", file_pair[1], X_vN_anomaly_scores, true_labels_list[1], best_anomaly_threshold)
        calculate_and_plot_detection_rate("X_tN", file_pair[3], X_tN_anomaly_scores, true_labels_list[3], best_anomaly_threshold)
        calculate_and_plot_detection_rate("X_vNA", file_pair[2], X_vNA_anomaly_scores, true_labels_list[2], best_anomaly_threshold)
...
def calculate_rec_error_vecs(model, X_vN1, scaler):
    ...
    for i in range(len(X_vN1)):
        ...
        error_vecs.append(np.subtract(current_sequence,
                                      predicted_sequence))  #took np.absolute() out because i don't think it is useful here
...
def compute_anomaly_score(error_vecs, mu,
                          sigma):  #todo: Need to calculate anomaly score for each error_vec; not sure if this does this but otherwise just do it with a loop
    # print("Mu: " + str(mu))
    ...
    error_vecs = error_vecs.reshape(error_vecs.shape[0], error_vecs.shape[1] * error_vecs.shape[
        2])  #todo: this might be totally incorrect but idrk rn
...
def calculate_and_plot_detection_rate(dataset_name, file_path, anomaly_scores, true_labels, best_anomaly_threshold):
    ...
    median_anomaly_score_of_actual_anomalies = np.median(anomaly_scores[true_labels.flatten() == 1])
    print("median anomaly score of anomalies: " + str(median_anomaly_score_of_actual_anomalies))
    median_anomaly_score_of_normals = np.median(anomaly_scores[true_labels.flatten() == 0])
    print("median anomaly score of normals: " + str(median_anomaly_score_of_normals))
    print("\n---------------------------------------------------------------------------------\n")
    plot_data_over_threshold(anomaly_scores, true_labels, best_anomaly_threshold, file_path)
